...
def run():
    ...
    transactions_df = pd.read_csv(
        f"{data_dir}/transactions_df.csv", parse_dates=["trade_datetime"]
    )
    snapshots_df = pd.read_csv(
        f"{data_dir}/snapshots_df.csv", parse_dates=["snapshot_date"]
    )
    ...
    with col1:
        sns.histplot(
            data=ltv_segment_df, x="ltv", bins=50, kde=True, color="skyblue", ax=ax1
        )
    ...
    date_choice = st.date_input("세그먼트를 선택하세요", value="2024-01-01")
    transactions_df["trade_datetime"] = pd.to_datetime(
        transactions_df["trade_datetime"]
    )
    ...
    if date_choice:
        ...
        joined_df = pd.merge(
            selected_fee_sum, fee_sum, on="user_id", how="left"
        ).fillna(0)
        st.markdown(
